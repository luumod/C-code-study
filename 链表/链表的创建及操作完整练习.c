...
//前面是链表的操作，后面是链表的有头与无头的详细创建过程！！！！！！！！！！！！！！！！！

/*
注意，链表的创建分为有头节点和无头结点的创建，而每种创建又可以有头插法和尾插法创建
有头节点的头插法创建:	Node* s=(Node*) malloc(sizeof(Node))
s->next=head->next;	(新创建的节点为第二个节点)
head->next=s;
无头结点的头插法:
s->next=head;	(新创建的节点即为第一个节点)
head=s;

有头节点的尾插法创建:  r=head 一个尾指针控制位置 
r->next=s;
r=s;
无头结点的尾插法创建: 
if (head==NULL)
...
//}
//int main(void) {
//	Node* head1 = createlistH();
//	Node* head2 = createlistR();
//	show(head1);
//	printf("\n");
//	show(head2);
//	return 0;
